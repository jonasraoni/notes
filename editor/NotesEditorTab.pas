...
uses
  ...
  SynEdit, SynEditKeyCmds, NotesHighlighter, ComCtrls, StdCtrls, NotesUtils,
  NotesEditorTabPosList, SynEditTypes, NotesSmartIndent,
  NotesCodeCompletitionDlg;
...
type
  ...
  private
    ...
    FSmartIndent: TNotesSmartIndent;
    FFileName: string;
    fFileType: string;
    ...
    procedure setSmartIndent(const Value: boolean);
  ...
  public
    ...
    { Reindenta um bloco de código }
    procedure ReindentBlock;
    { Comenta o bloco.}
  ...
  published
    ...
    { Se true o Notes vai fazer uam espécie de "SmartIndent". }
    property UseSmartIndent: boolean read fUseSmartIndent write setSmartIndent;
...
Var
  ...
  CodeCompletition: TNotesCodeCompletitionDlg;
...
function strTabsToSpaces(const S: string; const TabSize: integer): string;
begin
  Result:= StringReplace(S, #9, StringOfChar(#32, TabSize), [rfReplaceAll]);
end;

function strSpacesToTabs(const S: string; const TabSize: integer): string;
begin
...
begin
  ...
  FHighlighter:= nil;
  FSmartIndent:= TNotesSmartIndent.Create;
  FSmartIndent.Editor:= FEditor;
...
begin
  ...
  if FSmartIndent <> nil then
    FreeAndNil( FSmartIndent );
...
begin
  if (fUseSmartIndent) and (Command = ecChar) then
    FSmartIndent.CheckTrigger(AChar);
...
var
  ...
  function CheckIfShouldSkip(Key: Char): boolean;
  var
    Len: integer;
    S: string;
  begin
    ...
    S:= FEditor.LineText;
    Len:= length(S);
    if (Len > 1) and (FEditor.CaretX <= Len) then
      result:= (Key = S[FEditor.CaretX]);
  end;

  function CheckIfShouldAutoCompleteChars(Key: Char): boolean;
  var
    Len: integer;
    S: string;
    I, C: integer;
...
begin
  ...
  begin
    ...
    if CheckIfShouldSkip(Key) then
    begin
      Key:= #0;
      fEditor.CaretX:= fEditor.CaretX + 1;
    end else
    if CheckIfShouldAutoCompleteChars(Key) then
    begin
      Case Key of
        '(':
          begin
...
begin
  ...
  if (C = #0) and (Len > 0) and (fEditor.CaretX <= Len) then
  begin
   C:= S[FEditor.CaretX];
    if not (C in BracketSet) then
      C:= #0
    else
      FirstBracket:= FEditor.CaretXY;
  end;
...
begin
  if (Col < 1) or (length(FEditor.LineText) >= Col) then
    FEditor.CaretX:= Col
  else
    FEditor.ExecuteCommand(ecLineEnd, #0, nil);
end;
...
begin
  ...
  begin
    ...
    begin
      if BE.Char > 1 then
        BE.Char:= length( strTabsToSpaces(FEditor.Lines.Strings[BE.Line-1], FEditor.TabWidth) )
      else
      begin
        if BE.Line > 1 then
          BE.Line := BE.Line - 1;
    ...
    begin
      ...
      for I:= BB.Line to BE.Line do
        BE.Char:= Max( BE.Char, length( strTabsToSpaces(FEditor.Lines.Strings[I-1], FEditor.TabWidth)) +1);
    end;
    ...
    S:= strTabsToSpaces(S, FEditor.TabWidth);
    ...
    try
      ...
      if AdjustForTabs then
        S:= strSpacesToTabs(S, FEditor.TabWidth);
      ...
      if (FEditor.SelectionMode = smNormal) or (BB.Line = BE.Line) then
        BE.Char:= length( strTabsToSpaces(FEditor.Lines.Strings[BE.Line-1], FEditor.TabWidth) ) +1
      else
        for I:= BB.Line to BE.Line do
          BE.Char:= Max( BE.Char, length( strTabsToSpaces(FEditor.Lines.Strings[I-1], FEditor.TabWidth)) +1);
...
begin
  ...
  FSmartIndent.LoadFromFile( nProfile.Paths.FileTypesDir + addSlash(fFileType) + fFileType + NOTES_SMART_INDENT_EXT  );
  FSmartIndent.UseTabs:= not GetTabAsSpace;
  FSmartIndent.IndentSize:= FIndent;
end;
...
begin
  ...
  begin
    ...
    eventServer.NotifyEvent(self, 'filetypechanged', fFileType);
  end;
end;
...
var
  S: string;
begin
  eventServer.NotifyEvent(self, 'beforefilesave', AFile);
  ...
  eventServer.NotifyEvent(self, 'afterfilesave', AFile);
end;
...
procedure TNotesEditorTab.SpacesToTabs;
var
  S: string;
begin
  if fEditor.SelAvail then
  begin
    S:= StringOfChar(#32, fEditor.TabWidth);
    fEditor.SelText:= StringReplace(fEditor.SelText, S, #9, [rfReplaceAll]);
  end;
end;

procedure TNotesEditorTab.TabsToSpaces;
var
  S: string;
begin
  if fEditor.SelAvail then
  begin
    S:= StringOfChar(#32, fEditor.TabWidth);
    fEditor.SelText:= StringReplace(fEditor.SelText, #9, S, [rfReplaceAll]);
  end;
end;
...
begin
  ...
  while (S[tagBegin] <> '<') and (tagBegin > 0) do
  begin
    if S[tagBegin] in ['>', '/', '\'] then
    begin
  ...
  if (tagBegin+1 < Len) and (S[tagBegin+1] = '/') then
  begin
    Result:='>';
    exit;
  end;
...
procedure TNotesEditorTab.setSmartIndent(const Value: boolean);
begin
  fUseSmartIndent := Value;
  if Value then
    AutoIndent:= false;
end;
...
var
  ...
  IncInicio, IncFim, Int: Integer;

  // String usada como Auxilio para Contar Tokens
  sAux: string;
begin
  // fEscopeIndicator := eiEditor;

  fEscopePos.EscopeBegin.Y := 0;
  fEscopePos.EscopeBegin.X := 0;
  fEscopePos.EscopeEnd.Y := 0;
  fEscopePos.EscopeEnd.X := 0;
  ...
  begin
    ...
    Fim := Self.FEditor.CaretY - 1;

    sAux := Self.FEditor.Lines.Strings[Inicio];

    if (ExistValueIn(fEscopeStart, sAux, Inicio, Int)) or
      (ExistValueIn(fEscopeEnd, sAux, Inicio, Int)) then
      Exit;
    ...
    begin
      ...
      begin
        if (Length(sAux) - StartX) = Length(Copy(sAux, StartX+1, Length(sAux))) then
          sAux := Copy(sAux, StartX+1, Length(sAux))
        else
          sAux := EmptyStr;
        Dec(IncInicio);
      end;
    ...
    begin
      ...
      begin
        if (Length(sAux) - EndX) = Length(Copy(sAux, EndX+1, Length(sAux))) then
          sAux := Copy(sAux, EndX+1, Length(sAux))
        else
          sAux := EmptyStr;
        Dec(IncFim);
      end;
...
procedure TNotesEditorTab.IndentLine;
begin
  FSmartIndent.IndentLine;
end;

procedure TNotesEditorTab.ReindentBlock;
var
  I, sLine, eLine: integer;
begin
  if fEditor.SelAvail then
  begin
    sLine:= fEditor.BlockBegin.Line;
    eLine:= fEditor.BlockEnd.Line;
    fEditor.BeginUpdate;
    try
      fEditor.SelLength:= 0;
...
initialization
  ...
  CodeCompletition:= TNotesCodeCompletitionDlg.Create(nil);

finalization
  FreeandNil(CodeCompletition);
