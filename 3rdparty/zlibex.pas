...
type
  ...
  private
    ...
    FSize: Cardinal;    
  protected
    constructor Create( const AFile: PFile );
  ...
  TZCompressionBlock = class(TCustomBlockIO)
  private
    function GetCompressionRate: Single;
  public
    constructor Create( const dest: PFile; compressionLevel: TZCompressionLevel = zcDefault);
  ...
  TZDecompressionBlock = class(TCustomBlockIO)
  public
    constructor Create( const source: PFile );
    destructor Destroy; override;
...
constructor TCustomBlockIO.Create(const AFile: PFile);
begin
  FFile := AFile;
  FFilePos := FilePos( FFile^ );
end;

procedure TCustomBlockIO.DoProgress;
begin
  if Assigned(FOnProgress) then
    FOnProgress(Self);
end;

procedure TCustomBlockIO.SetPosition(const Position: Cardinal);
begin
  Seek( FFile^, Position );
end;
...
constructor TZCompressionBlock.Create(const dest: PFile;
  compressionLevel: TZCompressionLevel);
begin
  inherited Create( dest );
...
destructor TZCompressionBlock.Destroy;
begin
  FZStream.next_in := nil;
  FZStream.avail_in := 0;

  try
    if FilePos( FFile^ ) <> FFilePos then System.Seek( FFile^, FFilePos );

    while ZCompressCheck(deflate(FZStream, Z_FINISH)) <> Z_STREAM_END do
    begin
      BlockWrite( FFile^, FBuffer, SizeOf(FBuffer) - FZStream.avail_out );
      FZStream.next_out := FBuffer;
      FZStream.avail_out := SizeOf(FBuffer);
    end;
...
function TZCompressionBlock.GetCompressionRate: Single;
begin
  if FZStream.total_in = 0 then result := 0
  else result := (1.0 - (FZStream.total_out / FZStream.total_in)) * 100.0;
end;

function TZCompressionBlock.Seek(offset: Integer; origin: Word): Longint;
begin
  if (offset = 0) and (origin = soFromCurrent) then
  begin
    result := FZStream.total_in;
...
function TZCompressionBlock.Write(const buffer; count: Integer): Longint;
var
  BytesCount: Cardinal;
begin
  FZStream.next_in := @buffer;
  ...
  begin
    ...
    if FZStream.avail_out = 0 then
    begin
      BlockWrite( FFile^, FBuffer, SizeOf(FBuffer), BytesCount );
      Inc( FSize, BytesCount );
...
{ TZDecompressionBlock }

constructor TZDecompressionBlock.Create(const source: PFile);
begin
  inherited Create(source);
  FZStream.next_in := FBuffer;
  FZStream.avail_in := 0;
...
destructor TZDecompressionBlock.Destroy;
begin
  inflateEnd(FZStream);
  inherited Destroy;
end;

function TZDecompressionBlock.Read(var buffer; count: Integer): Longint;
var
  zresult: Integer;
begin
  FZStream.next_out := @buffer;
  ...
  begin
    if FZStream.avail_in = 0 then
    begin
      BlockRead( FFile^, FBuffer, SizeOf( FBuffer ), FZStream.avail_in );
  ...
  if (zresult = Z_STREAM_END) and (FZStream.avail_in > 0) then
  begin
    System.Seek( FFile^, FilePos( FFile^ ) - FZStream.avail_in );
    FFilePos := FilePos( FFile^ );
...
function TZDecompressionBlock.Seek(offset: Integer; origin: Word): Longint;
var
  buf: array[0..8191] of Char;
  i: Integer;
begin
  ...
  begin
    ...
    if offset > 0 then
    begin
      for i := 1 to offset div SizeOf(buf) do BlockRead( FFile^, buf, SizeOf( buf ) );
      BlockRead( FFile^, buf, offset mod SizeOf( buf ) );
    end;
  end
  else if (offset = 0) and (origin = soFromEnd) then
  begin
